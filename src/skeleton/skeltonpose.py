...
def process_images_in_folder(input_folder, output_csv="skeleton_coords.csv", overwrite=True, backup_folder=None):
    ...
    if not image_paths:
        print("이미지 파일을 찾지 못했습니다. 폴더에 .jpg/.png/.jpeg 파일이 있는지 확인하세요.")
        return
    ...
    def safe_imread(path):
        """Windows에서 한글 경로를 포함한 파일도 안전하게 읽도록 처리.

        내부적으로 numpy.fromfile + cv2.imdecode를 사용합니다.
        """
        try:
    ...
    def safe_imwrite(path, img):
        """cv2.imwrite가 실패하는 환경을 위해 imencode + tofile 방식으로 저장."""
        try:
    ...
    with mp_pose.Pose(static_image_mode=True, min_detection_confidence=0.5) as pose:
        for p in image_paths:
            ...
            # CSV용 레코드: 오직 목(nose), 좌/우 어깨, 좌/우 엉덩이 좌표만 저장
            rec = {"image_path": str(p.name)}
            for i in UPPER_BODY_LANDMARKS:
                # 정규화 좌표
                rec[f"lm_{i}_x"] = float(landmarks[i][0])
                rec[f"lm_{i}_y"] = float(landmarks[i][1])
                # 픽셀 좌표
                rec[f"lm_{i}_x_px"] = int(landmarks[i][0] * w)
                rec[f"lm_{i}_y_px"] = int(landmarks[i][1] * h)
            # 상체 랜드마크 정규화 좌표 저장
            for i in UPPER_BODY_LANDMARKS:
                rec[f"lm_{i}_x"] = float(landmarks[i][0])
                rec[f"lm_{i}_y"] = float(landmarks[i][1])
...
if __name__ == '__main__':
    ...
    try:
        process_images_in_folder(args.input_folder, output_csv=args.result, overwrite=not args.no_overwrite, backup_folder=args.backup)
